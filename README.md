...
一个聚焦火山引擎 `Seedance 2.0 / Seedream 5.0` 与阿里云 `Wan 2.7 Video / Wan 2.7 Image` 的双厂商 AIGC 控制台。

GitHub:
`https://github.com/lcnwys/seedance2.0-wan2.7-AIGC-Dashboard`

## 1. 项目介绍

### 我为什么做这个项目

我是阿里云、火山云方向的服务商，平时会接触很多想用最新模型能力做业务验证、内容生产、工作流搭建的客户。

但真实情况是，很多客户并不缺需求，缺的是技术团队、部署能力和一套能快速跑起来的后台。  
所以我做这个项目，最初并不是为了“做一个很炫的开源仓库”，而是为了让自己触达的、没有技术开发能力的客户，也能更快用上 `Seedance 2.0` 和 `Wan 2.7` 的图片 / 视频能力。
...
- 双厂商统一接入
  一套后台同时管理火山引擎与阿里云模型，方便做对比、切换和团队共用。

- 视频与图片统一工作台
  视频生成、图片生成、素材资产、任务列表、费用统计在同一个后台完成。

- 团队维度配置 API Key
  不依赖单一全局 Key，可以按团队配置火山 / 阿里云 API Key，更适合多人协作和商用场景。

- 同时兼容 TOS 与 OSS
  图片、视频生成结果与上传素材可以统一转存到对象存储，方便沉淀自己的资产。
...
这个仓库不是一个“堆满所有能力的大而全平台”，而是一个更适合实际交付、部署和二开的双厂商 AIGC 基础盘。
...
- 如果你用火山存储，就填 `TOS_*`
- 如果你用阿里云存储，就填 `OSS_*`
- `REMOTE_ASSET_MAX_BYTES` 控制远端结果转存大小上限，默认 5 GiB；TOS fallback 与 OSS 都会走服务端流式中转
...
```bash
git clone https://github.com/lcnwys/seedance2.0-wan2.7-AIGC-Dashboard.git
...
我比较重视口碑，也希望大家合作时是“规则透明、预期稳定、沟通直接”的状态，这样大家才放心。

### 微信 / 联系方式

![微信二维码占位图](./docs/wechat-qrcode-placeholder.png)
...
- 如果你要继续深度部署，请查看 [DEPLOY.md](./DEPLOY.md)
- 如果你要对项目做二开，建议优先保留现在这套双厂商、团队化、对象存储抽象结构
- 如果你要把它包装成自己的官网或渠道版，首页右下展示位和 README 联系方式都可以替换成你自己的内容
