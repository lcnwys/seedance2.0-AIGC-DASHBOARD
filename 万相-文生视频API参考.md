...
| ##### 请求体（Request Body） |
| **model** `*string*` **（必选）** 模型名称。模型列表与价格详见[百炼控制台](https://bailian.console.alibabacloud.com/cn-beijing?tab=model#/model-market/all)。 示例值：wan2.7-t2v。 |
...
| **X-DashScope-Async** `*string*` **（必选）** 异步处理配置参数。HTTP请求只支持异步，**必须设置为**`**enable**`。 **重要** 缺少此请求头将报错：“current user api does not support synchronous calls”。 |
| ##### 请求体（Request Body） |
| **model** `*string*` **（必选）** 模型名称。模型列表与价格详见[百炼控制台](https://bailian.console.alibabacloud.com/cn-beijing?tab=model#/model-market/all)。 示例值：wan2.6-t2v。 |
